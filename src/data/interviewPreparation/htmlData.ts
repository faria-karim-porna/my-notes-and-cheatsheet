...
export const htmlData: QuestionsAndAnswersType[] = [
...

  {
    ...
    answers: [
      `In HTML, tags are the building blocks used to define the structure of a webpage. They are enclosed in angle brackets (< >) and come in pairs: opening tags and closing tags. Opening tags indicate the beginning of an element, while closing tags mark the end. Some tags, known as self-closing or void tags, do not have a corresponding closing tag. Tags provide instructions to web browsers about the content.`,
    ],
  },
...

  {
    ...
    answers: [
      `Formatting in HTML refers to the way to control the appearance of content. For example, the <b> tag is used to make text bold, <i> tag is for italicizing text, and <u> tag is for underlining text.`,
    ],
  },

  {
    ...
    answers: [
      `The HTML contains six types of headings which are defined with the <h1> to <h6> tags. Each type of heading tag displays different size of texts. <h1> is the largest heading tag and <h6> is the smallest one.`,
    ],
  },
